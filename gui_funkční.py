...
from logic_varpro import run_NCBI_variant_analysis  # Nová logika s EVA a NCBI (run_NCBI_variant_anaysis)
    ...
    def analyze_varpro(self):
        try:
            ...
            if not all([email, organism, chromosome, amplicon_positions, snp_pos_str, assembly]):
                messagebox.showwarning("Warning", "Vyplňte prosím všechna pole v RENDOVarPro.")
                return
            ...
            self.varpro_results_text.insert(tk.END, "Stahuji FASTA z NCBI a varianty z EVA, prosím čekejte...\n")
            self.root.update_idletasks()

            # Zavoláme logiku
        
            result = run_ncbi_variant_analysis(
                email,
                organism,
                "", 
                amplicon_positions,
            ...
            self.varpro_fasta_text.insert(tk.END, result.get("fasta_sequence", ""))

            self.varpro_variant_file_text.delete("1.0", tk.END)
            vcf_content = result.get("variant_file_content", "")
            self.varpro_variant_file_text.insert(tk.END, vcf_content)

            self.varpro_results_text.delete("1.0", tk.END)
            analysis_text = result.get("analysis_results", "Analýza dokončena.")
            self.varpro_results_text.insert(tk.END, analysis_text)

        except Exception as e:
            messagebox.showerror("Error", f"Analýza RENDOVarPro selhala: {e}")
